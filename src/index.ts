/**
 * ke-voice-mcp · 壹克的嗓子
 * Railway deployment (Node.js + Express + MCP Streamable HTTP)
 */
...
async function generateAudio(text: string): Promise<{
  success: boolean;
  audio_base64?: string;
  error?: string;
}> {
  try {
    ...
    const response = await fetch(url, {
      ...
      body: JSON.stringify({
        ...
        voice_settings: {
          ...
          style: parseFloat(process.env.ELEVENLABS_STYLE || "0.9"),
        },
      }),
    });
    ...
    if (!response.ok) {
      ...
      return { success: false, error: `ElevenLabs ${response.status}: ${err}` };
    }

    const buf = await response.arrayBuffer();
    const bytes = new Uint8Array(buf);
    return { success: true, audio_base64: Buffer.from(bytes).toString("base64") };
  } catch (e) {
    return {
      success: false,
...
// =============================================================================
// Audio Player HTML (WeChat-style inline player)
// =============================================================================
...
function createVoiceServer(): McpServer {
  const botName = process.env.BOT_NAME || "壹克";
    ...
    async ({ text }: { text: string }) => {
      ...
      return {
        content: [
          {
            type: "text" as const,
            text: `Voice generation failed: ${result.error}`,
          },
        ],
        structuredContent: {
...
<html><head><meta charset="utf-8"><title>ke-voice-mcp</title>
<style>body{font-family:system-ui;max-width:600px;margin:40px auto;padding:20px;color:#333;line-height:1.6}
h1{color:#07c160}code{background:#f5f5f5;padding:2px 8px;border-radius:4px;font-size:14px}</style>
</head><body>
<h1>ke-voice-mcp</h1>
<p>MCP voice server for inline audio playback in Claude.</p>
<p>Add this URL as a Claude Connector:</p>
<code>${req.protocol}://${req.get("host")}/mcp</code>
<p style="margin-top:16px"><a href="/status">Status</a></p>
</body></html>`);
});
...
app.listen(PORT, "0.0.0.0", () => {
  console.log(`ke-voice-mcp listening on :${PORT}`);
});
